function analyse3d(inputFilename, aBoneExample, aCavityExample, anImplantExample, avoidEdgeDistance, minSlice, maxSlice, radiiRegionBorders, halfEdgeSize, filterRadius, maxIter, maxDistance, SHOWRESULT, SAVERESULT, origo, R, marks, outputFilenamePrefix, nRadiiRegionPoints)
...
for nR = 1:nRegions
    for ii = 1:size(marks,1)-1
        ...
        x3RegionOfInterest                 = x3RegionOfInterst3d(newVol, minSlice, maxSlice, []);
        radiiRegion                        = radiiRegionBorders(nR, :);
        [bone, cavity, neither, distances] = fraction3d(dstMap, x3RegionOfInterest, rotatedBoneMask, rotatedCavityMask, rotatedNeitherMask, maxDistance, nRadiiRegionPoints, radiiRegion);
...
if SAVERESULT
    fprintf('INFO::\t\tSaving results\n');
    save([outputFilenamePrefix,'fractions.mat'], 'fractions');
    fprintf('INFO::\t\tDone saving results\n');
end
