...
for fn = 1:length(fileCell)
    ...
    area = zeros(nBands, 1);
    boneCavityFraction = zeros(stackSize, 1);
    ...
    for ii = 2:stackSize
        ...
        dstMap = sgnDstFromImg(implantMask);
        dstBorders = [dstMap(1:s, 1) dstMap(1:s, s) dstMap(1, 1:s)' dstMap(s, 1:s)'];
        maxRadius = max(floor(min(dstBorders)));
        ...
        csvwrite(sprintf('boneVolume_%d.csv', ii), boneVolume)
        csvwrite(sprintf('area_%d.csv', ii), area)
        save(['../segmentations/' dataset '_v6_double.mat'], 'savedImplantMasks', 'savedBoneMasks', '-v6')
    end
